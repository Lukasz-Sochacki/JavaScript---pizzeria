...
export default class RangeSlider {
    ...
    dom.forEachAncestors(
      ...
      (el) =>
        (isEventOnSlider =
          ...
          !dom.hasClass(el, this.options.disabledClass)),
      true
    );
  ...
  _addVerticalSlideScrollFix() {
    if (this.vertical && !verticalSlidingFixRegistered) {
      document.addEventListener(
        'touchmove',
        RangeSlider._touchMoveScrollHandler,
        { passive: false }
      );
  ...
  _setPosition(pos) {
    ...
    // Snapping steps
    let value = this._getValueFromPosition(
      func.between(pos, 0, this.maxHandleX)
    );
    ...
    if (typeof e[pagePositionProperty] !== 'undefined') {
      pageOffset =
        e.touches && e.touches.length
          ? e.touches[0][pagePositionProperty]
          : e[pagePositionProperty];
      ...
      e.touches &&
      e.touches[0] &&
      typeof e.touches[0][pagePositionProperty] !== 'undefined'
    ) {
      pageOffset = e.touches[0][pagePositionProperty];
    } else if (
      e.currentPoint &&
      (typeof e.currentPoint.x !== 'undefined' ||
        typeof e.currentPoint.y !== 'undefined')
    ) {
  ...
  _getValueFromPosition(pos) {
    ...
    const value =
      this.step * Math.round((percentage * (this.max - this.min)) / this.step) +
      this.min;
